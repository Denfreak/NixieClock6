...
/* Преобразование и коррекция температуры
 * Входные параметры: нет
 * Выходные параметры: нет
 */
void tconvcorr() {
  int _temp = (int)(temp_event.temperature * 10) + tempcorr;

  if (_temp > 999) _temp = 999;
  if (_temp < 0) _temp = 0;

  indiDigits[2] = (byte)(_temp % 10);
  _temp /= 10;
  indiDigits[1] = (byte)(_temp % 10);
  _temp /= 10;
  indiDigits[0] = (byte)(_temp % 10);
}
  ...
  switch (curMode) {
    ...
    case SETALARM:                                // (3) установка времени будильника
      ...
      if (!(curMode == SETALARM && !alm_set)) {
        ...
        if (btnR.isHolded()) {                     // увеличить значение на 5 - ЗАМЕНА БЕЗ ЗАМЕНЫ ВЕРСИИ
          if (!currentDigit) {
            changeHrs += 5;
    ...
    case SHTEMP:                                  // (4) отображение температуры (5 сек)
      if (measurementsTimer.isReady()) {          // обновление показаний
        if (isBMEhere) { 
          bme_temp->getEvent(&temp_event);
          bme_pressure->getEvent(&pressure_event);
          bme_humidity->getEvent(&humidity_event);
      ...
      if (btnA.isHolded()) retToTime();         // удержание сенсора
      if (btnSet.isClick()) isFreeze = !isFreeze; // переключение freeze
      if (isFreeze && isBMEhere) {              // в режиме freeze можно корректировать температуру, есои она вообще есть
        if (btnSet.isHolded()) {                // удержание кнопки установки
          tempcorr = 0;                         // обнуление коррекции
          tconvcorr();                          // обновление температуры
          EEPROM.put(TCORR, tempcorr);
        }
        if (btnR.isClick()) {                   // нажатие кнопки увеличения
          tempcorr++;                           // увеличение температуры на десятые
          if (tempcorr > 100) tempcorr = 100;
          tconvcorr();                          // обновление температуры
          EEPROM.put(TCORR, tempcorr);
        }
        if (btnL.isClick()) {                   // нажатие кнопки уменьшения
          tempcorr--;                           // уменьшение температуры на десятые
          if (tempcorr < -100) tempcorr = -100;
          tconvcorr();                          // обновление температуры
          EEPROM.put(TCORR, tempcorr);
        }
        if (btnR.isHolded()) {                  // удержание кнопки увеличения
          tempcorr += 10;                       // увеличение температуры на единицы
          if (tempcorr > 100) tempcorr = 100;
          tconvcorr();                          // обновление температуры
          EEPROM.put(TCORR, tempcorr);
        }
        if (btnL.isHolded()) {                  // удержание кнопки уменьшения
          tempcorr -= 10;                       // уменьшение температуры на единицы
          if (tempcorr < -100) tempcorr = -100;
          tconvcorr();                          // обновление температуры
          EEPROM.put(TCORR, tempcorr);
        }
...
#else
          if ((int)humidity_event.relative_humidity == 100) indiDigits[2] = indiDigits[3] = 9;
          else {
